...
float stripCurrentDraw(Adafruit_NeoPixel &strip, float current_per_channel) {  // fairly sure this is correctly implemented
...
float stripPowerDraw(Adafruit_NeoPixel &strip, float current_per_channel, float voltage) {
  return stripCurrentDraw(strip, current_per_channel) * voltage;
}
...
void stripApplyBrightness(Adafruit_NeoPixel &strip, float brightness) {
  ...
  for (int n = 0; n < numLEDs; n++) {
    ...
    g = ((color >> 8) & 0x0000ff);
    b = (color & 0x0000ff);
    strip.setPixelColor(n, (int)(r*brightness), (int)(g*brightness), (int)(b*brightness));
  }
}
...
uint32_t interpolateColors888(uint32_t col_1, uint32_t col_2, float frac) {
  uint8_t r1 = (col_1 & 0xff0000) >> 16;
  uint8_t r2 = (col_2 & 0xff0000) >> 16;
  uint8_t g1 = (col_1 & 0x00ff00) >> 8;
  uint8_t g2 = (col_2 & 0x00ff00) >> 8;
...
uint32_t fadeColor(uint32_t color888, float factor) {
  uint8_t r = (uint8_t)(((color888 >> 16) & 0x0000ff) * factor);
  uint8_t g = (uint8_t)(((color888 >>  8) & 0x0000ff) * factor);
  uint8_t b = (uint8_t)((color888         & 0x0000ff) * factor);
  return (r << 16 | g << 8 | b);
}

uint8_t squareWave(float time, float period) {
  if (fmod(time, period) < 0.5f * period)
    return 1;
  else
    return 0;
}